      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
            ...
            {!isLoading && topFloat.map((s, i) => (
              <div key={s.ticker} className="flex items-center gap-2 p-2 rounded border border-border/40 bg-muted/10">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-muted-foreground">유동주식: {fmt.float(s.floatShares)}</p>
                </div>
              </div>
            ))}
            {!isLoading && topFloat.length === 0 && <EmptyState message="유동회전율 데이터가 없습니다" />}
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            {!isLoading && topRvol.map((s, i) => (
              <div key={s.ticker} className="flex items-center gap-2 p-2 rounded border border-border/40 bg-muted/10">
                ...
                <div className="flex-1">
                  <span className="font-mono font-bold text-xs">{s.ticker}</span>
                  <p className="text-[10px] text-muted-foreground">{fmt.vol(s.volume)} 주</p>
                </div>
                <div className="text-right">
                  <p className="font-mono font-bold text-xs text-accent">{fmt.multiplier(s.derived.rvol)}</p>
                  <p className="text-[10px] text-muted-foreground">평균: {fmt.vol(s.avgVolume30)}</p>
                </div>
              </div>
            ))}
            {!isLoading && topRvol.length === 0 && <EmptyState message="RVOL 데이터가 없습니다" />}
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            {!isLoading && topGap.map((s, i) => (
              <div key={s.ticker} className="flex items-center gap-2 p-2 rounded border border-border/40 bg-muted/10">
                ...
                <div className="flex-1">
                  <span className="font-mono font-bold text-xs">{s.ticker}</span>
                  <p className="text-[10px] text-muted-foreground">시가: {fmt.price(s.open)}</p>
                </div>
                <div className="text-right">
                  <p className="font-mono font-bold text-xs text-bull">+{(s.derived.gapPercent * 100).toFixed(1)}%</p>
                  <p className="text-[10px] text-muted-foreground">전일: {fmt.price(s.prevClose)}</p>
                </div>
              </div>
            ))}
            {!isLoading && topGap.length === 0 && <EmptyState message="갭 상승 종목이 없습니다" />}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            ...
            {!isLoading && premarket.length === 0 && <EmptyState message="프리마켓 급등주가 없습니다" />}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            ...
            {!isLoading && squeeze.length === 0 && <EmptyState message="공매도 스퀴즈 후보가 없습니다" />}
          </div>
        </div>
      </div>
    </div>
